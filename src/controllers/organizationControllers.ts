...
import { organizationNamechema, organizationSchema, organizationStyleSchema } from "../schemas/organizationSchema";
import { getPrismaClient } from "../lib/prisma";
import { config } from "dotenv";
import { newOrganization } from "../utils/newOrganization";

config();
const prisma = getPrismaClient('fullnext_management');
const pump = promisify(pipeline);

export const organization = async (request: FastifyRequest, reply: FastifyReply) => {
    const { channels, industries, user, name } = organizationSchema.parse(request.body);
    ...
    try {
        ...
        await newOrganization(name, user)

        const pgClientNew = new Client({
        ...
        try {
            await pgClientNew.query('BEGIN'); // Inicia a transação
            ...
            await pgClientNew.query('COMMIT'); // Confirma a transação
            console.log("Sequências ajustadas com sucesso.");
        } catch (error) {
            await pgClientNew.query('ROLLBACK'); // Desfaz em caso de erro
            console.error("Erro ao ajustar sequências:", error);
        } finally {
            await pgClientNew.end(); // Fecha a conexão
        }
...
export const uploadOrganizacao = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
        const { name, color } = organizationStyleSchema.parse(request.query);
        ...
        if (organizationStyle) {
            if (organizationStyle.logoUrl) {
                try {
                    unlinkSync(organizationStyle?.logoUrl);
                } catch (err) {
...
export const getOrganizacao = async (request: FastifyRequest, reply: FastifyReply) => {
    const { name } = organizationNamechema.parse(request.params);
    try {
        const organization = await prisma.organization.findFirst({ where: { name } });
...
export const getAllOrganization  = async (request: FastifyRequest, reply: FastifyReply) => {
